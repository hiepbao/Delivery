...
using Delivery.Patterns.DecoratorPattern;
using PagedList;
...
namespace Delivery.Controllers
{
    public class NhanViensController : BaseController
    {
        ...
        private INhanVienValidator validator;
        ...
        {
            // Tạo validator cơ bản
            validator = new NhanVienValidator();

            // Áp dụng ValidationDecorator
            validator = new ValidationDecorator(validator);

            if (ModelState.IsValid && validator.Validate(nhanVien))
            {
                db.NhanVien_Add(nhanVien.TenNhanVien, nhanVien.NgaySinh, nhanVien.Email, nhanVien.SoDienThoai, nhanVien.MaKhuVuc).SingleOrDefault();
                //if (test != "Hợp lệ")
                //{
            ...
            TempData["FailMessage"] = "Thêm thất bại";
